...
Note: Page analysis now uses a split OCR + semantic architecture. Nova Pro
receives structured text maps (not images) and returns block IDs (not bounding
box coordinates). The prompt accepts an additional {entity_context} variable
...
# Max tokens for page analysis. Reduced from 8192 — Nova now outputs compact
# block_ids instead of bounding box coordinate objects, so responses are smaller.
# Still generous for dense pages with many redactions.
...
def get_prompt(type, page_text=None, guidelines=None, doc_summary=None, page_number=None, guidelines_text=None, entity_context=None):
    """
    ...
    Args:
        ...
        page_text: Unused. Retained for backward compatibility.
        ...
        entity_context: Formatted cross-page entity information for consistency
            (for page_analysis). Generated by ocr_extraction.format_entity_index_for_prompt.
    ...
    try:
        ...
        match type:
            ...
            case "page_analysis":
                ...
                # Page content is now passed as an OCR text map in the user message.
                # entity_context provides cross-page consistency information.
                prompt = page_analysis.page_analysis_prompt.format(
                    guidelines=guidelines,
                    document_summary=doc_summary or "No document summary available",
                    page_number=page_number,
                    entity_context=entity_context or "(No cross-page entity data available)"
                )
