...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # GET /guidelines/{guideline_id}/rules - Get rules for completed/reviewed guideline (admin only)
        elif http_method == 'GET' and path.endswith('/rules') and path_parameters.get('guideline_id'):
            if not is_admin:
                return build_api_response(403, {"error": "Forbidden", "message": "Admin access required"}, error=True)
            guideline_id = path_parameters['guideline_id']
...
def handle_get_guideline_rules(guideline_id: str) -> Dict[str, Any]:
    """Handle GET /guidelines/{guideline_id}/rules - Get rules for completed/reviewed guideline"""
    logger.info(f"Retrieving rules for guideline: {guideline_id}")

    guideline = get_guideline_rules(guideline_id)

    return build_api_response(200, {
        "success": True,
        "guideline_id": guideline_id,
        "guidelines_content": guideline['guidelines_content'],
