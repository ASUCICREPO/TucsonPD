...
export default function App() {
  ...
  const [userRole, setUserRole] = useState<'admin' | 'officer' | null>('officer');
  ...
  const handleSignIn = (role: 'admin' | 'officer') => {
    setUserRole(role);
    if (role === 'admin') {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 'dashboard':
        return <DashboardScreen onStartNewCase={handleStartNewCase} onViewCase={handleViewCase} cases={cases} onUpdateCases={setCases} />;
      case 'intake-form':
        return (
          <IntakeFormScreen
            onBack={() => {
        ...
        <header className="bg-slate-900 text-white py-4 px-8">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="relative flex items-center gap-3">
              <span className="text-slate-300">{userRole === 'admin' ? 'Admin' : 'Records Officer'}</span>
              <button 
              ...
              {showProfileDropdown && (
                <div className="absolute right-0 top-12 w-48 bg-white rounded-md shadow-lg z-10 py-1">
                  <button 
                    className="flex items-center px-4 py-2 text-slate-700 hover:bg-slate-100 w-full text-left transition-colors" 
